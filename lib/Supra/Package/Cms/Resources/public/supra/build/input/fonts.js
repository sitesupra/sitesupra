...
YUI.add('supra.input-fonts', function (Y) {
	...
	Y.extend(Input, Supra.Input.Proto, {
		 ...
		 * @private
		 */
		_updateFontList: function (_group) {
			// Prevent loop caused by 'sync'
			if (this._isUpdatingFontList) return;
			this._isUpdatingFontList = true;
			
			var group = typeof _group == 'string' ? _group : this.widgets.slideshow.get('slide'),
		...
		_setValues: function (values) {
			// Filter out groups, which don't have any fonts
			var filtered = [],
				i = 0,
				ii = values ? values.length : 0;
			
			for (; i < ii; i++) {
				if (values[i].fonts.length) {

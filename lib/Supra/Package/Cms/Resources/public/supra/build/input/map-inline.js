...
YUI.add('supra.input-map-inline', function (Y) {
	...
	Y.extend(Input, Supra.Input.Proto, {
		...
		_createMap: function (targetNode) {
			...
			// If there already is a map, then destroy it by creating
			// a new node and removing old one;
			// If we will reuse existing node, then that will screw up
			// google maps
			if (cont.get('childNodes').length) {
				var tmp = cont.getDOMNode().cloneNode();
				cont.insert(tmp, 'before');
				cont.remove(true);
				cont = new Y.Node(tmp);
			}
	...
	var MapManager = Supra.Input.MapManager = {
		...
		create: function (doc, win) {
			...
			win[fn] = function () { self.checkReady(doc, win, guid); };
		...
		checkReady: function (doc, win, guid) {
			if (win.google && win.google.maps && win.google.maps.LatLng) {
				// Google Maps loaded
				this.ready(guid);
			} else {
		...
		ready: function (guid) {
			...
			// Small delay so that if there are frontend listeners which 
			// create map they would create it
			Supra.later(16, this, function () {
				for(; i<ii; i++) {

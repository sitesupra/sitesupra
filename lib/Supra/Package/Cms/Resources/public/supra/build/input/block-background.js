...
YUI.add("supra.input-block-background", function (Y) {
	...
	Y.extend(Input, Supra.Input.Proto, {
		...
		/**
		 * Returns section button label
		 *
		 * @returns {String} Button label
		 * @protected
		 */
		getSectionButtonLabel: function (_value) {
			var value = typeof _value === 'undefined' ? this.get('value') : _value,
				title = value ? value.title || Supra.getObjectValue(value, ['image', 'filename']) : null;
			
			return title || Supra.Intl.get(["form", "block", "set_image"]);
		},
		...
		_uiSync: function (_value) {
			...
			if (this.widgets) {
				...
				if (this.widgets.buttonSet) {
					this.widgets.buttonSet.set("label", this.getSectionButtonLabel(value));
				}

	...
	Supra.HTMLEditor.addPlugin("icon", defaultConfiguration, {
		...
		setIconProperty: function (id, value, node) {
			...
			if (id == "align") {
				...
				node.removeClass(ALIGN_CLASS_NAMES.left).removeClass(ALIGN_CLASS_NAMES.right).removeClass(ALIGN_CLASS_NAMES.center);
				
				if (value) {
					ancestor.addClass(ALIGN_CLASS_NAMES[value]);
					node.addClass(ALIGN_CLASS_NAMES[value]);
				}
			} else if (id == "width") {
		...
		insertIcon: function (event) {
			...
			if (!htmleditor.get("disabled") && htmleditor.isSelectionEditable(htmleditor.getSelection())) {
				...
				if (this.selected_icon) {
					...
					htmleditor.replaceSelection('<span class="' + classname + (icon.align ? ' ' + ALIGN_CLASS_NAMES[icon.align] : '') + '" contenteditable="false" style="width: ' + icon.width + 'px; height: ' + icon.height + 'px;">' + html + '</span>');
		...
		untagHTML: function (html, data) {
			...
			html = html.replace(/\{supra\.icon id="([^"]+)"}/ig, function (tag, id) {
				...
				if (icon.isDataComplete()) {
					var classname = self.configuration.wrapperClassName + " " + (icon.align ? ALIGN_CLASS_NAMES[icon.align] : "");
					var svg = icon.toHTML({
						'id': id 
					});

	...
	Supra.HTMLEditor.addPlugin("image", defaultConfiguration, {
			...
			var form_config = {
				"inputs": [
					...
					{"id": "align", "style": "minimal", "type": "SelectList", "label": Supra.Intl.get(["htmleditor", "image_alignment"]), "value": "right", "values": [
						...
						{"id": "center", "title": Supra.Intl.get(["htmleditor", "alignment_center"]), "icon": "/public/cms/supra/img/htmleditor/align-center-button.png"},
		...
		setImageProperty: function (id, value, image) {
			...
			if (id == "title") {
				...
				ancestor.removeClass(ALIGN_CLASS_NAMES.left).removeClass(ALIGN_CLASS_NAMES.right).removeClass(ALIGN_CLASS_NAMES.center);
				image.removeClass(ALIGN_CLASS_NAMES.left).removeClass(ALIGN_CLASS_NAMES.right).removeClass(ALIGN_CLASS_NAMES.center);
				
				if (value) {
					ancestor.addClass(ALIGN_CLASS_NAMES[value]);
					image.addClass(ALIGN_CLASS_NAMES[value]);
				}
		...
		getImageWrapperNode: function (image) {
			...
			if (!ancestor) {
				...
				if (data.align) {
					ancestor.addClass(ALIGN_CLASS_NAMES[data.align]);
				}
		...
		insertImage: function (event) {
			...
			if (!htmleditor.get("disabled") && htmleditor.isSelectionEditable(htmleditor.getSelection())) {
				...
				if (this.selected_image) {
					...
					htmleditor.replaceSelection('<span class="' + classname + ' ' + ALIGN_CLASS_NAMES[data.align] + '" contenteditable="false" style="width: ' + data.crop_width + 'px; height: ' + data.crop_height + 'px;"><img draggable="false" id="' + uid + '" style="margin-left: -' + data.crop_left + 'px; margin-top: -' + data.crop_top + 'px;" width="' + data.size_width + '" src="' + src + '" title="' + Y.Escape.html(data.title) + '" alt="' + Y.Escape.html(data.description) + '" class="' + ALIGN_CLASS_NAMES[data.align] + '" /></span>');
		...
		dropImage: function (target, image_id) {
			...
			img = Y.Node.create('<span class="' + classname + ' ' + ALIGN_CLASS_NAMES[data.align] + '" contenteditable="false" style="width: ' + data.crop_width + 'px; height: ' + data.crop_height + 'px;"><img draggable="false" id="' + uid + '" style="margin-left: -' + data.crop_left + 'px; margin-top: -' + data.crop_top + 'px;" width="' + data.size_width + '" src="' + src + '" title="' + Y.Escape.html(data.title) + '" alt="' + Y.Escape.html(data.description) + '" class="' + ALIGN_CLASS_NAMES[data.align] + '" />');
		...
		untagHTML: function (html, data) {
			...
			html = html.replace(/\{supra\.image id="([^"]+)"}/ig, function (tag, id) {
				...
				if (src) {
					...
					var classname = self.configuration.wrapperClassName + " " + (item.align ? ALIGN_CLASS_NAMES[item.align] : "") + " " + item.style;

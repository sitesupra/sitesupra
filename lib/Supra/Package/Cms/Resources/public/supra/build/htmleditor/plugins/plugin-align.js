	...
	Supra.HTMLEditor.addPlugin("align", defaultConfiguration, {
		...
		handleNodeChange: function (event) {
			...
			if (elements.length) {
				...
				if (Y.DOM.hasClass(elements[0], ALIGN_CLASS_NAMES.left)) {
					align = 'left';
				} else if (Y.DOM.hasClass(elements[0], ALIGN_CLASS_NAMES.right)) {
					align = 'right';
				} else if (Y.DOM.hasClass(elements[0], ALIGN_CLASS_NAMES.center)) {
					align = 'center';
				} else if (Y.DOM.hasClass(elements[0], ALIGN_CLASS_NAMES.justify)) {
					align = 'justify';
				}
		...
		exec: function (data, command) {
			if (!this.silentUpdating && command === "align") {
				...
				if (elements.length) {
					...
					for (; i<ii; i++) {
						Y.DOM.removeClass(elements[i], ALIGN_CLASS_NAMES.left);
						Y.DOM.removeClass(elements[i], ALIGN_CLASS_NAMES.right);
						Y.DOM.removeClass(elements[i], ALIGN_CLASS_NAMES.center);
						Y.DOM.removeClass(elements[i], ALIGN_CLASS_NAMES.justify);
						
						Y.DOM.addClass(elements[i], ALIGN_CLASS_NAMES[data]);
					}

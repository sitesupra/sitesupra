...
Supra('supra.input', 'supra.slideshow', 'linkmanager.sitemap-linkmanager-node', 'supra.medialibrary', function (Y) {
	...
	new Action(Action.PluginLayoutSidebar, {
		...
		setDisplayMode: function (mode, selectable) {
			...
			// Reset tree
			if (this.tree) {
				this.tree.detach('render:complete', this.markNodeOnTreeLoadComplete, this);
			}
		},
				...
				case 'page':
					...
					if (key) {
						...
						if (!node || reloading_tree) {
							this.tree.on('render:complete', this.markNodeOnTreeLoadComplete, this, {'key': key, 'value': value});
						} else {
		...
		/**
		 * When tree completes loading try marking page
		 * 
		 * @param {Object} e Event facade object
		 * @param {Object} data Page search criteria
		 * @protected
		 */
		markNodeOnTreeLoadComplete: function (e, data) {
			var tree = this.tree,
				node = tree.getNodeBy(data.key, data.value);
			
			tree.set('selectedNode', null);
			
			if (node) {
				tree.set('selectedNode', node);
				
				//Update scrollbars
				tree.get('boundingBox').closest('.su-scrollable-content').fire('contentresize');
				tree.detach('render:complete', this.markNodeOnTreeLoadComplete, this);
			}
		},
